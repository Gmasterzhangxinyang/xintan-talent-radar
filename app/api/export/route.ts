...
export async function GET(request: Request) {
  ...
  const where: string[] = [];
  const values: string[] = [];
  for (const [parameter, column] of [["source", "source"], ["type", "intelligence_type"], ["priority", "priority"], ["taskId", "task_id"], ["intent", "intent"], ["status", "review_status"]] as const) {
  ...
  const headers = ["任务ID", "来源平台", "来源URL", "发布时间", "作者公开昵称", "作者公开ID", "原文片段", "AI提取标签", "求职意向等级", "线索类型", "优先级", "匹配度", "企业情报备注", "AI证据", "处理状态"];
  const body = rows.results.map((row) => [row.task_id, row.source, row.url, row.published_at, row.author, row.author_id, row.snippet,
    (() => { try { return JSON.parse(String(row.tags)).join("、"); } catch { return row.tags; } })(), row.intent, row.intelligence_type,
    row.priority, row.score, row.company_note, row.evidence, row.review_status]);
